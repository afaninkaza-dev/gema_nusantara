...
// Hapus simpan jika ada request
if (isset($_GET['hapus']) && is_numeric($_GET['hapus'])) {
    $cerita_id = (int) $_GET['hapus'];
    $q_hapus = $conn->prepare("DELETE FROM simpan WHERE user_id = ? AND cerita_id = ?");
    $q_hapus->bind_param("ii", $user_id, $cerita_id);
    $q_hapus->execute();
    header("Location: dashboardsimpan_user.php");
    exit;
}
            ...
            <?php if ($result && $result->num_rows > 0): ?>
                    ...
                    <?php while ($cr = $result->fetch_assoc()): ?>
                        <a class="cerita_wrapper" href="detailbuku.php?id=<?= $cr['id'] ?>">
                            ...
                            <div class="rating">
                                ...
                                <i class="fas fa-bookmark icon-small saved" title="Hapus Simpanan"
                                    onclick="event.preventDefault(); window.location='dashboardsimpan_user.php?hapus=<?= $cr['id'] ?>'"></i>
                            </div>
                        </a>
                    <?php endwhile; ?>
                </div>
                <?php if ($total_halaman > 1): ?>
                    <div class="pagination">
                        ...
                        <?php endfor; ?>
                        <?php if ($halaman < $total_halaman): ?><a
                                href="?halaman=<?= $halaman + 1 ?>">&raquo;</a><?php endif; ?>
                    </div>
                <?php endif; ?>
            <?php else: ?>
                <p class="kosong">Belum ada cerita yang disimpan.</p>
            <?php endif; ?>
        </div>
    </div>
</body>
